...
export class Git {
  ...
  async remove(options: { dir: string; filepath: string }): Promise<void> {
    const { dir, filepath } = options;
    this.config.logger?.info(`Removing file {dir=${dir},filepath=${filepath}}`);

    return git.remove({ fs, dir, filepath });
  }

  async status(options: { dir: string; filepath: string }): Promise<string> {
    const { dir, filepath } = options;
    this.config.logger?.info(`Status file {dir=${dir},filepath=${filepath}}`);

    return git.status({ fs, dir, filepath });
  }

  async listFiles(options: { dir: string }): Promise<string[]> {
    const { dir } = options;
    this.config.logger?.info(`listFiles file {dir=${dir}}`);
